...
function gfeedpausescroller(divid, divClass, delay, linktarget){
	...
	this.tickerdivheight=this.tickerdiv.currentStyle? parseInt(this.tickerdiv.currentStyle["height"])+40 : 0 //IE has trouble getting offsetWidth while page is loading, so use global CSS value instead. "40" accounts for any possible padding skewing things. Variable used by any other browser.
}
...
function formatrssmessage(feedslice, showoptions, itemcontainer, linktarget){
	...
	for (var i=0; i<feedslice.length; i++){ //Loop through the entered slice of a RSS feed (1 or more entries)
		var itemdescription=/description/i.test(showoptions)? ""+feedslice[i].content : /snippet/i.test(showoptions)? ""+feedslice[i].contentSnippet  : ""
		rssoutput+=itemcontainer + " " + itemdescription + itemcontainer.replace("<", "</") + "\n\n"
	}
